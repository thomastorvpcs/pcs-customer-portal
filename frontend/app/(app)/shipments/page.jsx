              ...
              {filtered.map((s) => (
                <tr
                  ...
                  className={`cursor-pointer transition-colors ${
                    selected?.id === s.id ? 'bg-blue-50/60 dark:bg-blue-900/20' : 'hover:bg-gray-50 dark:hover:bg-[#1a2540]'
                  }`}
                >
                  ...
                  <td className={`px-4 py-3 text-sm font-medium ${s.status === 'Exception' ? 'text-red-500' : 'text-gray-700 dark:text-gray-300'}`}>
                    {s.eta}
                  </td>
                </tr>
              ))}
        ...
        {selected && (
          <div className="flex-1 bg-white dark:bg-[#152035] rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 space-y-6">
                  ...
                  <div key={step.label} className="flex items-start flex-1">
                    <div className="flex flex-col items-center flex-1">
                      <div className="flex items-center w-full">
                        {i > 0 && (
                          <div className={`flex-1 h-0.5 -mr-1 ${!step.done ? 'bg-gray-200 dark:bg-gray-600' : trackingSteps[i - 1].done && !trackingSteps[i - 1].active ? 'bg-green-400' : 'bg-blue-400'}`} />
                        )}
                        ...
                        {i < trackingSteps.length - 1 && (
                          <div className={`flex-1 h-0.5 -ml-1 ${step.done && !step.active ? 'bg-green-400' : step.active ? 'bg-blue-400' : 'bg-gray-200 dark:bg-gray-600'}`} />
                        )}
                      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="px-4 py-2 text-sm font-medium bg-white dark:bg-[#1e2d45] border border-gray-200 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1a2540] transition-colors">
                View Order
              </button>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
              ...
              <button className="px-4 py-2 text-sm font-medium border border-orange-300 text-orange-500 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors">
                Manage Pickups
              </button>
            </div>
          </div>
