...
import { Search, SlidersHorizontal, Download, CheckCircle2, Circle, Truck, FileText, Package } from 'lucide-react'
...
const statusStyles = {
  ...
  'Pickup Ready': 'bg-orange-50 text-orange-600',
  ...
  return (
    <div className="flex-1 p-8 bg-[#f1f5f9]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-4 py-2 text-sm border border-gray-200 rounded-lg w-80 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
      ...
      <div className="flex gap-4">
        {/* Left: Shipment List */}
        <div className="w-[380px] flex-shrink-0 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {filtered.map((s) => (
                <tr
                  ...
                  className={`cursor-pointer transition-colors ${
                    selected?.id === s.id ? 'bg-blue-50/60' : 'hover:bg-gray-50'
                  }`}
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${statusStyles[s.status]}`}>
                      {s.status}
                    </span>
                  </td>
                  <td className={`px-4 py-3 text-sm font-medium ${s.status === 'Exception' ? 'text-red-500' : 'text-gray-700'}`}>
                    {s.eta}
                  </td>
                </tr>
              ))}
        ...
        {selected && (
          <div className="flex-1 bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-6">
            {/* Header */}
            <div>
              ...
              <p className="text-xs text-gray-400">
                Order: {selected.order} &nbsp;|&nbsp; {selected.carrier} &nbsp;|&nbsp; 12399AA1C123456784
              </p>
            </div>

            {/* Tracking Timeline */}
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-4">Tracking Timeline</h3>
                      ...
                      <div className="flex items-center w-full">
                        ...
                        <div className="flex-shrink-0 z-10">
                          {step.done ? (
                            ...
                            <Circle size={24} className="text-gray-300" fill="white" />
                          )}
                        </div>
              ...
              {/* Latest event */}
              <div className="mt-4 bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 flex items-start gap-3">
                <Truck size={15} className="text-blue-500 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-sm font-medium text-gray-800">Departed FedEx location - Memphis, TN</p>
            ...
            <div className="flex items-center gap-3">
              {[
                { label: 'BOL', icon: FileText },
                { label: 'Packing List', icon: Package },
              ].map(({ label, icon: Icon }) => (
                <button key={label} className="flex items-center gap-1.5 px-4 py-2 text-sm border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                  <Icon size={14} />
                  {label}
                </button>
              ))}
              <button className="px-4 py-2 text-sm font-medium bg-white border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                View Order
              </button>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100">
              <div>
                <p className="text-sm font-semibold text-gray-900">Pickup Authorization</p>
                <p className="text-xs text-gray-400 mt-0.5">Register drivers for customer pickup shipments</p>
              </div>
              <button className="px-4 py-2 text-sm font-medium border border-orange-300 text-orange-500 rounded-lg hover:bg-orange-50 transition-colors">
                Manage Pickups
              </button>
            </div>
          </div>
